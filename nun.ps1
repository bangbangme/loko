...
while (((Get-Date) - $startTime) -lt $timeout -and $promptsHandled -lt 2) {
    ...
    # Conditions to find either dialog
    $nameCondition1 = New-Object System.Windows.Automation.PropertyCondition([System.Windows.Automation.AutomationElement]::NameProperty, "Remote Desktop Connection")
    $nameCondition2 = New-Object System.Windows.Automation.PropertyCondition([System.Windows.Automation.AutomationElement]::NameProperty, "Remote Desktop Connection security warning")
    $nameCondition3 = New-Object System.Windows.Automation.PropertyCondition([System.Windows.Automation.AutomationElement]::NameProperty, "Connexion Bureau à distance")
    ...
    $dialogCondition = New-Object System.Windows.Automation.AndCondition(
        ...
        (New-Object System.Windows.Automation.OrCondition($nameCondition1, $nameCondition2, $nameCondition3))
    )
    $dialogElement = $rootElement.FindFirst([System.Windows.Automation.TreeScope]::Children, $dialogCondition)

    if ($dialogElement) {
        $windowName = $dialogElement.Current.Name
        Log "Found RDP dialog: '$windowName'"
        
        # Bring window to foreground to accept keystrokes
        $dialogElement.SetFocus()
        Start-Sleep -Milliseconds 250 # Brief pause for focus to shift

        if ($windowName -eq "Remote Desktop Connection security warning" -or $windowName -eq "Connexion Bureau à distance") {
            Log "Sending ALT+N to security warning..."
            [System.Windows.Forms.SendKeys]::SendWait("%n")
            [System.Windows.Forms.SendKeys]::SendWait("%c")
            $promptsHandled++
        }
        elseif ($windowName -eq "Remote Desktop Connection" -or $windowName -eq "Connexion Bureau à distance") {
            Log "Sending ALT+Y to connection dialog..."
            [System.Windows.Forms.SendKeys]::SendWait("%y")
            [System.Windows.Forms.SendKeys]::SendWait("%o")
            $promptsHandled++
        }
        
        Start-Sleep -Milliseconds 1000 # Wait for window to process keys
        continue
    }
