...
if (-not (Test-Path `$rdpFile)) { return }
...
while (`$stopwatch.Elapsed.TotalSeconds -lt `$timeoutSeconds -and -not `$finalWindowHidden) {
    `$callback = {
        ...
        if (`$pId -eq `$script:mstscId) {
            # This window belongs to our RDP process. Hide it immediately.
            [Win32Stealth]::HideWindowAggressively(`$hWnd)
            
            # Now, check if this is the final, connected window by checking its title.
            `$sb = New-Object System.Text.StringBuilder 256
            [Win32Stealth]::GetWindowText(`$hWnd, `$sb, `$sb.Capacity) | Out-Null
            `$windowTitle = `$sb.ToString()
            
            if (`$windowTitle -eq `$script:serverName) {
                # Found the main window. Signal that we can stop polling.
