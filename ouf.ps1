...
# --- Wait for RemoteApp window to spawn ---
Start-Sleep -Seconds 6  # give mstsc enough time

# --- Hide the RemoteApp window by title ---
powershell -Command "
...
$h=[W]::FindWindow($null,'Administrator: Windows PowerShell ISE (x86)');
if($h -ne [IntPtr]::Zero){[W]::ShowWindow($h,0)}
"
