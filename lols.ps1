...
while (((Get-Date) - $startTime) -lt $timeout -and $promptsHandled -lt 2) {
    ...
    if ($dialogElement) {
        ...
        if ($windowName -eq "Remote Desktop Connection security warning" -or $windowName -eq $frenchTitle) {
            Log "Sending ALT+N to security/French dialog..."
            #[System.Windows.Forms.SendKeys]::SendWait("%n")
            [System.Windows.Forms.SendKeys]::SendWait("%c")
            $promptsHandled++
        }
        elseif ($windowName -eq "Remote Desktop Connection" -or $windowName -eq $frenchTitle) {
