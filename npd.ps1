        ...
        if ($textElement) {
            ...
            if ($promptsHandled -eq 0) {
                ...
                elseif ($windowName -eq $frenchTitle) {
                    ...
                    # Send ALT+O for 'Do not ask again' as fast as possible, before clicking
                    ...
                    if (Click-DialogButtonWin32 $windowName 'Continuer') {
                        ...
                        if ($continuerButton) {
                            if ($continuerButton.GetSupportedPatterns() -contains [System.Windows.Automation.InvokePattern]::Pattern) {
                                # Send ALT+O before UI Automation click
                                [System.Windows.Forms.SendKeys]::SendWait('%o')
                                ...
                                # Send ALT+O before SendKeys for button
                                [System.Windows.Forms.SendKeys]::SendWait('%o')
            ...
            elseif ($promptsHandled -eq 1) {
                ...
                if ($windowName -eq "Remote Desktop Connection") {
                    ...
                    if ($noButtonElement) {
                        ...
                        # Send ALT+D for 'Do not ask again' as fast as possible, before clicking
                        [System.Windows.Forms.SendKeys]::SendWait('%d')
                        if (Click-DialogButtonWin32 $windowName 'Yes') {
                            $promptsHandled++
                        } else {
                            ...
                            if ($yesButton) {
                                if ($yesButton.GetSupportedPatterns() -contains [System.Windows.Automation.InvokePattern]::Pattern) {
                                    # Send ALT+D before UI Automation click
                                    [System.Windows.Forms.SendKeys]::SendWait('%d')
                                    $invokePattern = $yesButton.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)
                                    $invokePattern.Invoke()
                                    $promptsHandled++
                                    ...
                                    $Win32Type::SetForegroundWindow($nativeHwnd) | Out-Null
                                    Start-Sleep -Milliseconds 50
                                    # Send ALT+D before SendKeys for button
                                    [System.Windows.Forms.SendKeys]::SendWait('%d')
                                    [System.Windows.Forms.SendKeys]::SendWait('%y')
                                    $promptsHandled++
                                }
