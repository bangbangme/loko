        ...
        if ($textElement) {
            ...
            if ($promptsHandled -eq 0) {
                ...
                if ($windowName -eq "Remote Desktop Connection security warning") {
                    Log "Trying Win32 click for 'Connect'..."
                    if (Click-DialogButtonWin32 $windowName 'Connect') {
                        $promptsHandled++
                    } else {
                        Log "Win32 click failed, trying UI Automation for 'Connect'..."
                        ...
                        $connectButton = $dialogElement.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $connectButtonCondition)
                        if ($connectButton) {
                            if ($connectButton.GetSupportedPatterns() -contains [System.Windows.Automation.InvokePattern]::Pattern) {
                                $invokePattern = $connectButton.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)
                                $invokePattern.Invoke()
                                $promptsHandled++
                            } else {
                                Log "'Connect' button does not support InvokePattern, falling back to SendKeys."
                                $Win32Type::SetForegroundWindow($nativeHwnd) | Out-Null
                                Start-Sleep -Milliseconds 150
                            ...
                            Log "'Connect' button not found, falling back to SendKeys."
                            $Win32Type::SetForegroundWindow($nativeHwnd) | Out-Null
                            Start-Sleep -Milliseconds 150
                            [System.Windows.Forms.SendKeys]::SendWait("%n")
                            $promptsHandled++
                ...
                elseif ($windowName -eq $frenchTitle) {
                    Log "Trying Win32 click for 'Continuer'..."
                    if (Click-DialogButtonWin32 $windowName 'Continuer') {
                        $promptsHandled++
                        $firstPromptWasFrench = $true
                    } else {
                        Log "Win32 click failed, trying UI Automation for 'Continuer'..."
                        $continuerButtonCondition = New-Object System.Windows.Automation.PropertyCondition(
                            [System.Windows.Automation.AutomationElement]::NameProperty, "Continuer"
                        )
                        $continuerButton = $dialogElement.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $continuerButtonCondition)
                        if ($continuerButton) {
                            if ($continuerButton.GetSupportedPatterns() -contains [System.Windows.Automation.InvokePattern]::Pattern) {
                                $invokePattern = $continuerButton.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)
                                $invokePattern.Invoke()
                                $promptsHandled++
                                $firstPromptWasFrench = $true
                            } else {
                                Log "'Continuer' button does not support InvokePattern, falling back to SendKeys."
                                $Win32Type::SetForegroundWindow($nativeHwnd) | Out-Null
                                Start-Sleep -Milliseconds 150
                            ...
                            Log "'Continuer' button not found, falling back to SendKeys."
                            $Win32Type::SetForegroundWindow($nativeHwnd) | Out-Null
                            Start-Sleep -Milliseconds 150
                            [System.Windows.Forms.SendKeys]::SendWait("%c")
                            $promptsHandled++
            ...
            elseif ($promptsHandled -eq 1) {
                ...
                if ($windowName -eq "Remote Desktop Connection") {
                    $noButtonCondition = New-Object System.Windows.Automation.PropertyCondition(
                        [System.Windows.Automation.AutomationElement]::NameProperty, "No"
                    )
                    $noButtonElement = $dialogElement.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $noButtonCondition)
                    if ($noButtonElement) {
                        Log "Trying Win32 click for 'Yes'..."
                        if (Click-DialogButtonWin32 $windowName 'Yes') {
                            $promptsHandled++
                        } else {
                            Log "Win32 click failed, trying UI Automation for 'Yes'..."
                        ...
                        Log "Found intermediate loading window (no 'No' button). Ignoring and waiting for final prompt."
                    }
                }
                elseif ($windowName -eq $frenchTitle) {
                    Log "Trying Win32 click for 'Oui'..."
                    if (Click-DialogButtonWin32 $windowName 'Oui') {
                        $promptsHandled++
                    } else {
                        Log "Win32 click failed, trying UI Automation for 'Oui'..."
                        $ouiButtonCondition = New-Object System.Windows.Automation.PropertyCondition(
                            [System.Windows.Automation.AutomationElement]::NameProperty, "Oui"
                        )
                        $ouiButton = $dialogElement.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $ouiButtonCondition)
                        if ($ouiButton) {
                            if ($ouiButton.GetSupportedPatterns() -contains [System.Windows.Automation.InvokePattern]::Pattern) {
                                $invokePattern = $ouiButton.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)
                                $invokePattern.Invoke()
                                $promptsHandled++
                            } else {
                                Log "'Oui' button does not support InvokePattern, falling back to SendKeys."
                                $Win32Type::SetForegroundWindow($nativeHwnd) | Out-Null
                                Start-Sleep -Milliseconds 150
                            ...
                            Log "'Oui' button not found, falling back to SendKeys."
                            $Win32Type::SetForegroundWindow($nativeHwnd) | Out-Null
                            Start-Sleep -Milliseconds 150
                            [System.Windows.Forms.SendKeys]::SendWait("%o")
                            $promptsHandled++
...
# --- Self-register to run at user login via registry (HKCU...Run) ---
...
if ($null -eq $current -or $current.$regName -ne $regValue) {
    ...
    Log "Registered script to run at login via registry: $regPath\\$regName"
} else {
    Log "Script already registered to run at login."
}

# --- Self-register to run at login via Startup folder shortcut ---
$scriptPath = $MyInvocation.MyCommand.Definition
$startup = [Environment]::GetFolderPath('Startup')
$shortcutPath = Join-Path $startup 'DebugRDP.lnk'
...
try {
    ...
    $shortcut.Save()
    Log "Created shortcut in Startup folder: $shortcutPath"
} catch {
