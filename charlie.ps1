...
# Find all windows belonging to the mstsc.exe process ID and hide them aggressively.
`$mstscId = `$proc.Id
`$handles = @()
`$callback = {
    param(`$hWnd, `$lParam)
    `$outId = 0
    [Win32Stealth]::GetWindowThreadProcessId(`$hWnd, [ref]`$outId) | Out-Null
    if (`$outId -eq `$mstscId) { `$script:handles += `$hWnd }
    return `$true
} -as ([Win32Stealth]::EnumWindowsProc)
