...
while (((Get-Date) - $startTime) -lt $timeout -and $promptsHandled -lt 2) {
    ...
    if ($dialogElement) {
        ...
        if ($textElement) {
            Log "Handling 'Cannot be verified' prompt. Sending ALT+Y..."
            ...
            elseif ($promptsHandled -eq 1) {
                ...
                # The "Remote Desktop Connection" title is ambiguous. Differentiate by button presence.
                # The final certificate prompt has a "No" button, the intermediate loading window has "Cancel".
                if ($windowName -eq "Remote Desktop Connection") {
                    ...
                    if ($noButtonElement) {
                        Log "Found 'No' button, confirming final certificate prompt. Sending ALT+Y..."
                        [System.Windows.Forms.SendKeys]::SendWait("%y")
                        $promptsHandled++
                    } else {
